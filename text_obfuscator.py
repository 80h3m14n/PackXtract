...
class StealthObfuscator:
    ...
    def _generate_junk_code(self):
        ...
        for _ in range(2):
            var_name = f"_{''.join(random.choices(string.ascii_lowercase, k=2))}{random.randint(1000, 9999)}"
            junk_code.append(
                f"{var_name} = [i for i in range({random.randint(5, 10)})]")
...
class ControlFlowObfuscator(ast.NodeTransformer):
    def visit_FunctionDef(self, node):
        ...
        for _ in range(2):
            # If wrapped_body is a list containing a single ast.Try, flatten it
            if len(wrapped_body) == 1 and isinstance(wrapped_body[0], ast.Try):
                wrapped_body = wrapped_body[0].body
            wrapped_body = [
                ast.Try(
                    body=wrapped_body,
                    handlers=[
        ...
        # Ensure node.body is always a list of ast.stmt
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python light_PackXtract.py <filename.py>")
        sys.exit(1)

    filename = sys.argv[1]
    if not filename.lower().endswith('.py'):
        print("[ERROR] Only Python (.py) files are supported by light_PackXtract.py.")
        sys.exit(1)
